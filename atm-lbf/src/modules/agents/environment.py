...
class ForagingEnv(Env):
    ...
    def _make_gym_obs(self):
        def make_obs_array(observation):
            ...
            # # sorting foods according to distance to self
            # foods = [f_position for _, f_position in enumerate(zip(*np.nonzero(observation.field)))]
            # foods = sorted(foods, key=lambda k: abs(k[0] - seen_players[0].position[0]) + abs(k[1] - seen_players[0].position[1]))
            # for i, (y, x) in enumerate(foods):
            #     obs[3 * i] = y
            #     obs[3 * i + 1] = x
            #     obs[3 * i + 2] = observation.field[y, x]
