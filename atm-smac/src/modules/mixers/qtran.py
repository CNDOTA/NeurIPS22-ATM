    ...
    def __init__(self, args):
        ...
        if self.arch == "coma_critic":
            ...
            if self.args.agent == 'memory':
                self.mem_hidden_dim = self.args.mem_slots * self.args.num_heads * self.args.head_size
                q_input_size = self.state_dim + self.mem_hidden_dim + self.n_actions
            else:
        ...
        if self.args.network_size == "small":
            ...
            self.V = nn.Sequential(nn.Linear(self.state_dim, self.embed_dim),
                                   ...
                                   nn.Linear(self.embed_dim, 1))
            if self.args.agent == 'memory':
                ae_input = self.mem_hidden_dim + self.n_actions
            else:
